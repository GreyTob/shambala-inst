export const data = {
  courses: [
    // {
    //   to: '/neurophysiology',
    //   photo: '',
    //   courseName: 'Нейрофизиология',
    //   descr: 'Короткое описание',
    //   date: 'Дата',
    //   longDescr: `Длинное описание Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit optio?`,
    //   place: 'г. МАГНИТОГОРСК',
    //   teachers: 'Ольга Зинатова',
    //   cost: 'Стоимость',
    //   format: 'Очно или онлайн',
    //   methodology: `Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit`,
    //   result:
    //     'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit',
    // },
    // {
    //   to: '/something',
    //   photo: '',
    //   courseName: 'Что-то еще...',
    //   descr: 'Короткое описание',
    //   date: 'Дата',
    //   longDescr: `длинное описание Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit optio?`,
    //   place: 'г. МАГНИТОГОРСК',
    //   teachers: 'Ольга Зинатова',
    //   cost: 'Стоимость',
    //   format: 'Очно или онлайн',
    //   methodology: `Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit`,
    //   result:
    //     'Lorem ipsum, dolor sit amet consectetur adipisicing elit. Harum officia placeat incidunt laborum consequatur, nobis nostrum accusantium quia reprehenderit',
    // },
    {
      to: '/acroyoga',
      photo: 'acroyoga',
      courseName: 'Инструктор "Акройоги"',
      descr: 'Курс для начинающих и действующих инструкторов по акройоге',
      date: '21-27 февраля 2022',
      longDescr: `Курс подойдёт и тем, кто только хочет начать вести классы по акройоге и действующим инструкторам.
        Если в вашем городе нет акройоги, будьте первыми- начните развивать это направление, а мы научим как это сделать`,
      place: 'г. МАГНИТОГОРСК',
      teachers: 'Ольга Зинатова и Светлана Кожевникова',
      cost: '25000 руб. (предоплату 5000 руб вносите до 1 января)',
      format: 'Очно или онлайн',
      methodology: `* Методика развития силы, выносливости, гибкости в группе;
      * Примеры построения тренировок;
      * Основные принципы безопасности, противопоказания и оказание первой помощи;
      * Психоэмоциональный аспект взаимодействия в группе;
      * Развитие, творчество и прогресс в акройоге`,
      result:
        'Вы научитесь строить тренировку не только "весело и задорно", но максимально полезно и эффективно',
    },
  ],
  socials: [
    {
      value: 'fb',
      href: 'https://touch.facebook.com/olga.zinatova',
      target: '_blank',
      rel: 'noopener',
    },
    {
      value: 'inst',
      href: 'https://www.instagram.com/shambhala_yoga/',
      target: '_blank',
      rel: 'noopener',
    },
    {
      value: 'vk',
      href: 'https://vk.com/yogashambala74',
      target: '_blank',
      rel: 'noopener',
    },
    {
      value: 'ok',
      href: 'https://ok.ru/profile/115087064096',
      target: '_blank',
      rel: 'noopener',
    },
  ],
  youTube: {
    value: 'youTube',
    href: 'https://www.youtube.com/channel/UCW0xVC_n-4S8U43-UvqA0iQ',
    target: '_blank',
    rel: 'noopener',
  },
  contacts: [
    {
      value: 'telegram',
      href: 'tg://resolve?domain=yogashambala',
      target: '',
      rel: '',
    },
    {
      value: 'whatsapp',
      href: 'https://wa.me/79049746832?text=Здравствуйте,%20Ольга%20',
      target: '',
      rel: '',
    },
    // {
    //   value: 'skype',
    //   href: 'Skype:login?chat',
    //   target: '',
    //   rel: '',
    // },
    {
      value: 'viber',
      href: 'viber://add?number=79049746832',
      target: '',
      rel: '',
    },
  ],
};
